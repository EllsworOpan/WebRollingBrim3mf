import { NS, xml, serialize, children, child, meta, num, matrix, readMesh, checkIds, type Doc, type El } from './three-mf-xml';
...
export function importPrusaProject(name: string, files: Record<string, Uint8Array>, modelPath: string, doc: Doc): Project {
...
export function exportPrusaProject(project: Project, result: BrimResult, modelDocument?: Doc): Uint8Array {
  ...
  const doc = modelDocument || (files[modelPath] ? xml(strFromU8(files[modelPath])) : xml(`<model unit="millimeter" xml:lang="en-US" xmlns="${NS}" xmlns:slic3rpe="http://schemas.slic3r.org/3mf/2017/06"><metadata name="slic3rpe:Version3mf">1</metadata><resources/><build/></model>`));
