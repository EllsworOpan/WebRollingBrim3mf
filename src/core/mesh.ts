...
export function transformMesh(mesh: Mesh, matrix: Matrix4): Mesh {
  ...
  for (let i = 0; i < mesh.vertices.length; i += 3) {
    p.fromArray(mesh.vertices, i).applyMatrix4(matrix); vertices.push(p.x, p.y, p.z);
  }
...
export function loadStl(bytes: ArrayBuffer): Mesh {
  ...
  let minZ = Infinity;
  for (let i = 2; i < vertices.length; i += 3) minZ = Math.min(minZ, vertices[i]);
  for (let i = 2; i < vertices.length; i += 3) vertices[i] -= minZ;
  return splitTouchingShells({ vertices, triangles: faces });
}
...
export function extrude(rings: Rings, height: number): Mesh {
  ...
  for (const polygon of polygonsOf(rings)) {
    ...
    const faces = ShapeUtils.triangulateShape(polygon.outer.map(p => new Vector2(p.x, p.y)), polygon.holes.map(r => r.map(p => new Vector2(p.x, p.y))));
