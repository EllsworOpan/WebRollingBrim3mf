...
interface Props { result: BrimResult; showBrim: boolean; showUncovered: boolean; compare: boolean; probe: boolean; fitKey: number; selected: string | null }
export default function FirstLayerView({ result, showBrim, showUncovered, compare, probe, fitKey, selected }: Props) {
  ...
  return <div className="layer-view"><svg ref={svg} role="img" aria-label="Mesh cross-sections and generated rolling brim" viewBox={`${view.x} ${view.y} ${view.w} ${view.h}`} onDoubleClick={fit}
    ...
    <g transform="scale(1,-1)">
      ...
      {showUncovered && result.objects.filter(object => object.uncovered.length).map(object => <g key={object.id} opacity={selected && selected !== object.id ? 0.3 : 1}>
        <path aria-label="Uncovered footprints" d={path(object.uncovered.flatMap(island => [island.outer,...island.holes]))} fill="url(#uncovered-hatch)" fillRule="evenodd" stroke="#ff8972" strokeWidth=".12"/>
      </g>)}
