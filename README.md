...
A local browser workbench for adding rolling-brim **mesh parts** to STL and 3MF models. Based on the UI and rolling-circle region classifier in the sibling `WebRollingBrim` project. The legacy Python script is not used.
...
1. Open one `.stl` or `.3mf`. A 3MF may contain multiple independent objects and instances.
...
3. Set the assumed first-layer height (default **0.2 mm**). The mesh is intersected at **half that height**, or **Z=0.1 mm** by default. The generated brim spans **Z=0–0.2 mm**. For PrusaSlicer projects with an absolute first-layer height, a button offers that value; it does not silently change the assumption.
4. Use **Compare base outlines** for sloping/flared bases. Cyan shows the section just above the bed; pink shows the section just below the layer top. The midpoint remains the outline used for generation. The app reports substantial area changes across the layer.
...
Validation artifacts go to ignored `.local/`, including `two-objects.3mf`, the slicer roundtrip, validation G-code, and `slicer-validation.json`. G-code exists only as a development check; the application accepts STL/3MF and exports only 3MF.
