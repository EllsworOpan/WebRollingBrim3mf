...
5. Check which objects should receive a brim. Every entry in the Objects panel is processed independently, as if it were the only object loaded with those settings. Other objects, their order and their selection do not affect its brim or circle sweep. Click an object name to highlight it.
...
- Model scaling is respected, including uniform/nonuniform scales, rotations, mirrors and X/Y tilts. Shared instances are sampled and exported independently using their own transforms. Brim height, width and gap are measured in final print millimetres. The exporter applies the inverse parent transform to the new brim, so scaling a model up does not scale a 0.2 mm brim into multiple layers. Rescaling the entire object afterward in the slicer also rescales its attached parts; regenerate from the original model at the new scale if the brim dimensions must remain fixed.
...
- Brims are never clipped to bed boundaries. All supported formats generate the complete brim and leave placement checks to the slicer. Existing native brim and general XY size compensation settings are retained, with notes where detected. Elephant-foot compensation is overridden to 0 per object.
...
Scale regressions independently decode exported world coordinates for enlarged, reduced, nonuniform, rotated, mirrored and tilted models, both single objects and shared instances across all four slicer adapters. They check preserved model geometry and real brim height/width at 0.2 and 0.3 mm. PrusaSlicer integration saves scaled inputs, reopens the outputs and slices both heights, asserting every brim extrusion stays on the first layer. Sweep-preview tests cover diameter versus brim width, holes/pockets, tight entrances, object independence and complete sweeps. Rolled-boundary tests independently measure a 5 mm band against the analytic arc of a 30 mm circle spanning a 20 mm gap within one object, including separation and overlap. They check the actual test plate at diameters below and above the gap width, connections within one object versus four independent objects, closed meshes, selection, full bands and absence of calculation-frame artifacts. Installed-slicer tests verify that connecting bands become first-layer perimeter extrusions within one object and that separate objects receive no such connection.

Object-independence tests import the same pieces as one STL, one 3MF component assembly and multiple 3MF build items. They compare each object's complete result against standalone generation across object order, selection, nearby models, widths and circle diameters. Additional cases cover objects inside another object's hole or pocket entrance, negative volumes restricted to their parent, all modifier roles, and coincident repeated instances. Exports retain both brim/brim overlaps and brim/model intersections. Repeated scaled, mirrored and tilted instances keep independent selection and the requested brim thickness. Native integration tests reopen these exports in PrusaSlicer 2.9.6, PrusaSlicer 3.0 alpha12, Bambu Studio and OrcaSlicer.
