...
2. Adjust rolling diameter, width, and separation gap. The circle traces a rolled boundary around the complete arrangement. Brim width is measured outward from that boundary, so it follows the curved entrances of blocked gaps and can connect nearby pieces even when a conventional brim of that width would not meet. Enclosed holes and narrow-entry pockets remain independent options, using the same reachability classifier as the G-code workbench. The width rule replaces the old model-distance clipping rule.
...
**Circle sweep** shows a translucent teal area where the full rolling circle fits, equivalent to a brim band as wide as the rolling diameter measured from the same rolled boundary. The orange band remains the actual brim. The sweep follows the hole/pocket toggles, object selection, neighbouring models and bed clipping; it does not change the exported mesh. A circle at the pointer provides a size reference. There is no center-path line.
...
- Brim ownership keeps local bands with their models and assigns each connecting section to the first adjacent selected object. Parts meet without overlapping or an artificial separation gap. Unselected objects still obstruct the circle; their local bands are reserved during assignment and then omitted. Rolling-circle accessibility and the combined band consider the whole arrangement, including disconnected shells within one object.
...
Scale regressions independently decode exported world coordinates for enlarged, reduced, nonuniform, rotated and mirrored models. They check preserved model geometry and real brim height/width at 0.2 and 0.3 mm. PrusaSlicer integration saves scaled inputs, reopens the outputs and slices both heights, asserting every brim extrusion stays on the first layer. Sweep-preview tests cover diameter versus brim width, holes/pockets, tight entrances, disabled neighbours and bed clipping. Rolled-boundary tests independently measure a 5 mm band against the analytic arc of a 30 mm circle spanning a 20 mm gap, including separation and overlap. They check the actual test plate at diameters below and above the gap width, identical combined geometry for one or four objects, closed meshes, non-overlapping ownership, selection, bed clipping and absence of calculation-frame artifacts. Installed-slicer tests verify the connecting bands become first-layer perimeter extrusions for both grouped and separate models.
